...
function GenerateTestDataNoise(cr, snr)
	task = 'Test';
	...
	for i = 1:length(newPaths)
		...
		for j = 1:1:numImage
				...
				v = var(rawImg(:));
				rawImg = imnoise(rawImg, 'gaussian', 0, v/snr);
	...
	psnrSingle = zeros(2, 1);
	ssimSingle = zeros(2, 1);
	elSingle = zeros(2, 1);
	
	frame1 = frame(:,:,1,190);
	frame1 = im2uint8(frame1);
	frame2 = frameLabel(:,:,1,190);
	frame2 = im2uint8(frame2);
	fig = imshow(frame1);
	saveas(fig, ['shotPut_recon_', num2str(cr), '_snr_', num2str(snr)], 'eps');
	fig = imshow(frame2);
	saveas(fig, ['shotPut_base_', num2str(cr), '_snr_', num2str(snr)], 'eps');
	[psnrSingle(1), ssimSingle(1), elSingle(1)] = measureFrame(frame1, frame2);
	
	frame1 = frame(:,:,1,40);
	frame1 = im2uint8(frame1);
	frame2 = frameLabel(:,:,1,40);
	frame2 = im2uint8(frame2);
	fig = imshow(frame1);
	saveas(fig, ['flute_recon_', num2str(cr), '_snr_', num2str(snr)], 'eps');
	fig = imshow(frame2);
	saveas(fig, ['flute_base_', num2str(cr), '_snr_', num2str(snr)], 'eps');
	[psnrSingle(2), ssimSingle(2), elSingle(2)] = measureFrame(frame1, frame2);
	...
	save(['measurement', num2str(cr), '_snr_', num2str(snr), '.mat'], ...
			  'psnrAvg', 'ssimAvg', 'elAvg', 'psnrSingle', 'ssimSingle', 'elSingle');

end

function [psnr1, ssim1, el1] = measureFrame(frame1, frame2)
	img = frame1;
	img = im2uint8(img);
	lab = frame2;
	lab = im2uint8(lab);
